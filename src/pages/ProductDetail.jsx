...
export default function ProductDetail() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 pt-28 sm:pt-32 pb-24">
      <div className="grid lg:grid-cols-2 gap-10 lg:gap-16">
            ...
            <button
              ...
              className={`px-4 py-2 font-tag text-xs uppercase tracking-widest border transition-colors ${
                viewMode === "photos"
                  ...
                  : "border-white/15 text-bone/70 hover:border-cyan hover:text-cyan"
              }`}
            >
              Photos
            </button>
            ...
            {color.images.map((img, i) => (
              <button
                ...
                className={`w-20 h-20 shrink-0 overflow-hidden border-2 transition-colors ${
                  viewMode === "photos" && imgIdx === i ? "border-cyan" : "border-transparent opacity-70 hover:opacity-100"
                }`}
              >
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-3">
            ...
            <span className="font-body text-sm text-mute">{product.category === "shoes" ? "Footwear" : "Apparel"}</span>
          </div>

          <div className="flex items-baseline gap-3 mt-5">
            <span className="font-tag text-2xl sm:text-3xl text-cyan font-bold">
            ...
            {discount > 0 && (
              <>
                ...
                <span className="font-tag text-volt text-sm font-bold">-{discount}%</span>
              </>
            )}
          </div>
              ...
              {product.colors.map((c, i) => (
                <button
                  ...
                  className={`w-10 h-10 rounded-full border-2 transition-all ${
                    colorIdx === i ? "border-cyan scale-110" : "border-white/20"
                  }`}
          ...
          <div className="mt-8">
            <div className="flex items-center justify-between mb-3">
              <p className="font-body font-bold text-sm">
                Size{sizeError && <span className="text-red-400 font-normal ml-2">— pick a size</span>}
              </p>
              <span className="font-tag text-xs text-mute uppercase">
                {product.category === "shoes" ? "UK" : ""}
              </span>
            </div>
              ...
              {product.sizes.map((s) => (
                <button
                  ...
                  className={`min-w-[3rem] px-3 py-2 font-tag text-sm border transition-colors ${
                    size === s
                      ? "bg-cyan text-ink border-cyan"
                      : "border-white/15 text-bone/80 hover:border-cyan hover:text-cyan"
                  }`}
                >
                  {s}
                </button>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mt-8">
            <div className="flex items-center border border-white/15 w-fit">
              <button
                ...
                className="px-3 py-3 hover:text-cyan"
            ...
            <button
              ...
              className="flex-1 bg-cyan text-ink font-display tracking-wide text-lg py-3 hover:bg-cyan-soft transition-colors flex items-center justify-center gap-2"
            >
              {added ? (
                <>
      ...
      <div className="mt-10">
        <Link to="/shop" className="font-body text-sm text-mute hover:text-cyan">
          ← Back to Shop
        </Link>
      </div>
    </div>
