              ...
              {items.map((item) => (
                <div key={item.lineId} className="flex gap-3 border-b border-white/5 pb-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="font-tag text-cyan text-sm mt-1">
                      {site.currencySymbol}{item.price.toLocaleString("en-IN")}
                    </p>
                    <div className="flex items-center gap-3 mt-2">
                      <button
                        onClick={() => updateQuantity(item.lineId, item.quantity - 1)}
                        className="w-6 h-6 flex items-center justify-center border border-white/20 hover:border-cyan"
            ...
            {items.length > 0 && (
              <div className="border-t border-white/10 p-5 space-y-3">
                ...
                <Link
                  to="/checkout"
                  onClick={() => setIsCartOpen(false)}
                  className="block text-center w-full bg-cyan text-ink font-display tracking-wide text-lg py-3 hover:bg-cyan-soft transition-colors"
                >
                  CHECKOUT
                </Link>
              </div>
